<div class="layout-wrapper layout-content-navbar">
    <div class="layout-container">
        @include('layouts.admin.menu',['menuprincipaleactive' =>'stock_vente','menuactive' =>'bon_entree'])
                            ...
                            <div class="card mb-12">
                                <div class="card-body">
                                    ...
                                    <div class="info-container">
                                        <form method="POST" id="add_bon_en_form" action="{{route('admin.stock_vente.bon_entre.update',$detail[0]['id'])}}" class="row g-3">
                                            @csrf
                                            ...
                                            <div class="col-12 col-md-4">
                                                ...
                                                <input
                                                    ...
                                                    name="date_be"
                                                    class="form-control"
                                                    value="{{$detail[0]['date_be'] ?? date('Y-m-d')}}"
                                                />
                                            </div>
                                            <div class="col-12 col-md-4">
                                                <label
                                                    class="form-label"
                                                    for="IDFOURNISSEUR"
                                                >{{ __('content.fournisseur') }}</label
                                                >
                                                <select id="IDFOURNISSEUR" name="IDFOURNISSEUR" class="select2 form-select">
                                                    <option value="{{$detail[0]['fournisseur_id']}}">
                                                        {{$raison_soc_fourn}}
                                                    </option>
                                                    @for($i=0;$i<count($liste_fournisseur);$i++)
                                                        @if($detail[0]['fournisseur_id'] != $liste_fournisseur[$i]['id'])
                                                            <option value="{{$liste_fournisseur[$i]['id']}}">{{$liste_fournisseur[$i]['raison_soc_fourn']}}</option>
                                                        @endif
                                                ...
                                                >
                                                    ...
                                                    @if(count($liste)>0)
                                                        @foreach($liste as $elem)
                                                            ...
                                                            <tr>
                                                                <td style="text-align:center">
                                                                    <input type="checkbox" name="record" value="{{$elem['IDPROD']}}"/>
                                                                </td>
                                                                <td style="text-align:center">
                                                                    <input type="text" id="code_ligne_entree_table" name="code_ligne_entree_table" class="form-control" value="{{$elem['code_prod']}}" readonly/>
                                                                </td>
                                                                <td style="text-align:center">
                                                                    {{$elem['des_prod']}}
                                                                </td>
                                                                ...
                                                                <td style="text-align:center">
                                                                    <input type="text" name="qte_entree_table" id="qte_entree_table{{$elem['IDPROD']}}" class="form-control" value="{{$elem['qte_entree'] ?? '0'}}" onkeyup="calcul_total({{$elem['IDPROD']}})"/>
                                                                </td>

                                                                <td style="text-align:center">
                                                                    <input type="text" name="pu_prod_entree_table" id="pu_prod_entree_table" class="form-control" value="{{$elem['pu_prod_entree'] ?? '0'}}"  onkeyup="calcul_total({{$elem['IDPROD']}})"/>
                                                                </td>
                                                                <td style="text-align:center">
                                                                    <input type="text" id="total_ligne_entree_table{{$elem['IDPROD']}}" name="total_ligne_entree_table" class="form-control" value="{{$elem['total_ligne_entree'] ?? '0'}}" readonly/>
                                                                </td>
                                            ...
                                            <div class="d-flex justify-content-center pt-3">
                                                ...
                                                <button type="submit" id="submitButton" class="btn btn-primary" onclick="submit_form()">{{ __('content.Modifier') }}</button>
                                            </div>
                                        </form>
