...
<section id="landingContact" class="section-py bg-body">
    <div class="container" style="margin-top: 100px">
        ...
        <h3 class="text-center mb-1">
            <span class="section-title">Trouver</span> des évènements
        </h3>
        <p class="text-center mb-4 mb-lg-5 pb-md-3">Trouvez des évènements selon vos besoins</p>
        <div class="row">
            <div class="col-md-12">
                <ul class="nav nav-pills flex-column flex-sm-row mb-4">
                    ...
                    @for($i=0;$i<count($list_cat);$i++)
                        @if($list_cat[$i]['id'] == $detail_cat[0]['id'])
                            <li class="nav-item">
                                <a class="nav-link active" href="{{route('evenement_by_categ',$list_cat[$i]['id'])}}">
                                    <i class='fa fa-minus me-1'></i> {{$list_cat[$i]['desc']}}
                                </a>
                            </li>
                        @else
                            <li class="nav-item">
                                <a class="nav-link" href="{{route('evenement_by_categ',$list_cat[$i]['id'])}}">
        ...
        <div class="col-md-12 col-xl-12 mb-12">
            <div class="card h-100">
                <div class="card-header d-flex align-items-center justify-content-between">
                    <h5 class="card-title m-0 me-2">{{ __('content.evenement') }} {{$detail_cat[0]['desc']}}</h5>
                    {{--                    <div class="dropdown">--}}
                ...
                <div class="card-body">
                    ...
                    >
                        <thead>
                        <tr>
                            <th hidden>Date deb</th>
                            <th hidden>Date fin</th>
                            <th hidden>heure deb</th>
                            <th hidden>heure fin</th>
                            <th hidden></th>
                            <th hidden></th>
                                ...
                                <tr>
                                    ...
                                    <td style="width: 50px">
                                        <div>
                                            ...
                                            <!-- {{ $carbondate =  \Carbon\Carbon::parse($elem['date_deb'])->timezone('Europe/Paris')}} -->
                                            ...
                                            <small class="text-muted">{{$elem['heure_deb']}} </small>
                                    ...
                                    <td>
                                        <div style="border-right: 1px solid lightgray;">
                                            ...
                                            <small class="text-muted">{{$elem['type_even_desc'] }}</small>
                                        </div>
                                    </td>
                                    <td style="width: 50px;">
                                        <div>
                                    ...
                                    <td style="width: 50px">
                                        <div>
                                            @if($elem['date_deb'] > date('Y-m-d'))
                                                <a href="{{route('evenement.details',$elem['id'])}}" class="btn btn-sm btn-icon" style="background-color: lightgray; width:100%;padding:15px; margin-bottom: 5px">
