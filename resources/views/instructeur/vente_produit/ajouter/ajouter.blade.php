<div class="layout-wrapper layout-content-navbar">
    <div class="layout-container">
        @include('layouts.instructeur.menu',['menuprincipaleactive' =>'vente_prod','menuactive' =>'add_vente_prod'])
                            ...
                            <div class="card mb-12">
                                <div class="card-body">
                                    ...
                                    <div class="info-container">
                                        <form method="POST" id="add_vente_form" action="{{route('instructeur.vente_prod.add')}}" class="row g-3">
                                            @csrf
                                            ...
                                            <div class="col-12 col-md-3">
                                                <label class="form-label" for="sexe">Soldé ?</label>
                                                <div class="col-md">
                                                    <div class="form-check form-check-inline mt-3">
                                                        <input class="form-check-input" type="radio" name="paiement_status" id="inlineRadio1" value="1" checked/>
                                                        <label class="form-check-label" for="inlineRadio1">{{ __('content.oui') }}</label>
                                                    </div>
                                                    <div class="form-check form-check-inline">
                                                        <input class="form-check-input" type="radio" name="paiement_status" id="inlineRadio2" value="0" />
                                                        <label class="form-check-label" for="inlineRadio2">{{ __('content.non') }}</label>
                                                    </div>
                                                </div>
                                            </div>

                                            <div class="col-12 col-md-3" id="payeeParBox">
                                                <label class="form-label" for="date">Payé par</label>
                                                <select id="paiement_par" name="paiement_par" class="select2 form-select">
                                                    <option value="Espèce">Espèce</option>
                                                    <option value="Konnect +">Konnect +</option>
                                                </select>
                                            </div>
                                            <script>
                                                function togglePayeeParBox() {
                                                    const isSolde = document.querySelector('input[name="paiement_status"]:checked').value;
                                                    const payeeParBox = document.getElementById('payeeParBox');

                                                    if (isSolde === "1") {
                                                        payeeParBox.style.display = "block";
                                                    } else {
                                                        payeeParBox.style.display = "none";
                                                    }
                                                }

                                                // Écoute le changement des boutons radio
                                                document.querySelectorAll('input[name="paiement_status"]').forEach((radio) => {
                                                    radio.addEventListener('change', togglePayeeParBox);
                                                });

                                                // Appel initial pour cacher/afficher au chargement
                                                document.addEventListener('DOMContentLoaded', togglePayeeParBox);
                                            </script>
